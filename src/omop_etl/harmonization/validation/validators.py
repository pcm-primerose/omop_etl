...
class StrictValidators:
    ...
    @staticmethod
    def validate_optional_float(value: Optional[float], field_name: str) -> Optional[float]:
        """Strict validation - only accepts int or None (already parsed)"""
        if value is None:
            return None
    ...
    @staticmethod
    def validate_optional_date(value: Optional[dt.date], field_name: str) -> Optional[dt.date]:
    ...
    def validate_optional_positive_int(value: Any, field_name: str) -> Optional[int]:
        ...
        if isinstance(value, int):
            if value < 0:
                raise ValueError(f"Exepected positive integer for {field_name}, got {value}")
            return value

        raise TypeError(f"Expected int or None for field {field_name}, got {type(value)} with value {value}")
    ...
    @staticmethod
    def validate_optional_date_range(value: Any, field_name: str, min_date: dt.date, max_date: dt.date) -> Optional[dt.date]:
        if value is None:
            return None

        if isinstance(value, dt.date):
            if not (min_date <= value <= max_date):
                raise ValueError(f"Date {value} is outside min date: {min_date} or max date: {max_date}")
            return value

        raise TypeError(f"Excpected datetime.date or None for field {field_name}, got {type(value)} with value {value}")
    ...
    def validate_optional_bool(value: Any, field_name: str) -> Optional[bool]:
        if value is None:
            ...
            raise TypeError(f"Excpected bool or None for field {field_name}, got {type(value)} with value {value}")

    @staticmethod
    def validate_optional_int_range(value: Optional[int], field_name: str, min_val: int, max_val: int) -> Optional[int]:
