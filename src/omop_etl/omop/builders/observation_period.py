...
class ObservationPeriodBuilder(OmopBuilder[ObservationPeriodRow]):
    ...
    table_name: ClassVar[str] = OmopTables.OBSERVATION_PERIOD

    def build(self, ctx: BuildContext) -> BuildResult[ObservationPeriodRow]:
        patient = ctx.patient
        ...
        observation_type = self.concepts.resolve("ecrf", domains={"Type Concept"})
        period_type_concept_id = observation_type[0].concept_id if observation_type else 0
