...
class DrugExposureBuilder(OmopBuilder[DrugExposureRow]):
	...
	def build(self, patient: Patient, person_id: int) -> list[DrugExposureRow]:
		print(f"[DEBUG] DrugExposureBuilder.build called for {patient.patient_id}, study_drugs={patient.study_drugs}, prev_count={len(patient.previous_treatments) if patient.previous_treatments else 0}, concom_count={len(patient.concomitant_medications) if patient.concomitant_medications else 0}")  # debug # To remove after testing
		rows = []
	...
	def _build_previous_treatment_row(self, patient: Patient, person_id: int, prev_treatment: PreviousTreatments, index: int, route: Optional[TreatmentCycle] = None) -> list[DrugExposureRow]:
