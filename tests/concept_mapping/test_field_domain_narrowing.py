...
def test_one_field_two_domains_both_resolve():
    ...
    drug = service.lookup_semantic("p1", FIELD, "Erivedge", domains={OmopDomain.DRUG})
    regimen = service.lookup_semantic("p1", FIELD, "Erivedge", domains={OmopDomain.REGIMEN})
...
def test_unconfigured_field_raises():
    ...
    with pytest.raises(ValueError, match="unconfigured field_path"):
        service.lookup_semantic("p1", other_field, "anything", domains={OmopDomain.DRUG})
...
def test_configured_but_unmapped_value_returns_empty():
    ...
    result = service.lookup_semantic("p1", FIELD, "not-a-real-drug", domains={OmopDomain.DRUG})
