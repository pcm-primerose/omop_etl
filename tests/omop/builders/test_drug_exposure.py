...
class TestDrugExposureBuilder:
    def test_table_name(self, static_index, structural_index, row_id_generator):
        assert DrugExposureBuilder(ConceptLookupService(static_index, structural_index), row_id_generator).table_name == "drug_exposure"
    ...
    def test_empty_patient_returns_empty(
        ...
        result = DrugExposureBuilder(ConceptLookupService(static_index, structural_index), row_id_generator).build(create_build_context(patient, PERSON_ID))
        assert result == BuildResult(rows=result.rows, publications=())


class TestTreatmentCycleRows:
    def test_iv_cycle_produces_row_with_concept_id_zero(
        self,
        static_index,
        structural_index,
        row_id_generator,
    ...
    def test_oral_cycle_produces_row(
        self,
        static_index,
        structural_index,
        row_id_generator,
    ...
    def test_oral_cycle_with_end_date_populates_days_supply_matching_cycle_length(
        self,
        static_index,
        structural_index,
        row_id_generator,
        ...
        row = (
            DrugExposureBuilder(ConceptLookupService(static_index, structural_index), row_id_generator).build(create_build_context(patient, PERSON_ID)).rows[0]
        )
    ...
    def test_end_date_falls_back_to_start_date(
        self,
        static_index,
        structural_index,
        row_id_generator,
        ...
        assert len(result.rows) == 1
        assert result.rows[0].drug_exposure_end_date == dt.date(2023, 3, 1)

    def test_no_cycle_type_omits_route_and_dose(
        self,
        static_index,
        structural_index,
        row_id_generator,
    ...
    def test_multiple_cycles_produce_multiple_result(
        self,
        static_index,
        structural_index,
        row_id_generator,
        ...
        assert result.rows[0].drug_exposure_id != result.rows[1].drug_exposure_id

    def test_combination_components_use_ingredient_not_source(
        self,
        static_index,
        structural_index,
        row_id_generator,
        ...
        result = DrugExposureBuilder(ConceptLookupService(static_index, structural_index, semantic), row_id_generator).build(
            create_build_context(patient, PERSON_ID)
        )

        assert len(result.rows) == 2
        concept_ids = {r.drug_concept_id for r in result.rows}
        assert concept_ids == {6001, 6002}
        assert result.rows[0].drug_exposure_id != result.rows[1].drug_exposure_id

    def test_with_semantic_match_uses_mapped_concept(
        self,
        static_index,
        structural_index,
        row_id_generator,
        ...
        assert len(result.rows) == 1
        assert result.rows[0].drug_concept_id == 1234


class TestSemanticLookupStrategy:
    def test_uses_ingredient_name_when_available(
        self,
        static_index,
        structural_index,
        row_id_generator,
        ...
        assert len(result.rows) == 1
        assert result.rows[0].drug_concept_id == 5001

    def test_falls_back_to_source_name_when_no_ingredient(
        self,
        static_index,
        structural_index,
        row_id_generator,
    ...
    def test_missing_mappable_data_returns_empty_list(
        self,
        static_index,
        structural_index,
        row_id_generator,
    ...
    def test_missing_source_treatment_name_maps_ingredient(
        self,
        static_index,
        structural_index,
        row_id_generator,
...
class TestPreviousTreatmentRows:
    ...
    so, can't default to emit "0-result" for previous treatments, unless
    ...
    def test_additional_treatment_maps_to_drug(
        self,
        static_index,
        structural_index,
        row_id_generator,
    ...
    def test_main_treatment_maps_to_drug(
        self,
        static_index,
        structural_index,
        row_id_generator,
        ...
        assert len(result.rows) == 1
        assert result.rows[0].drug_source_value == "Letrozole"

    def test_both_fields_map_to_drug_emits_two_result(
        self,
        static_index,
        structural_index,
        row_id_generator,
        ...
        assert len(result.rows) == 2
        assert result.rows[0].drug_exposure_id != result.rows[1].drug_exposure_id
        source_values = {r.drug_source_value for r in result.rows}
        assert source_values == {"Drug A", "Drug B"}

    def test_no_drug_mapping_produces_no_row(
        self,
        static_index,
        structural_index,
        row_id_generator,
    ):
        """Treatment fields without drug domain mappings produce no result."""
        patient = create_patient(PID, "test")
        prev = PreviousTreatment(patient_id=PID)
...
class TestConcomitantMedicationRows:
    def test_produces_row_with_concept_id_zero(
        self,
        static_index,
        structural_index,
        row_id_generator,
    ...
    def test_produces_row_with_mapped_concept(
        self,
        static_index,
        structural_index,
        row_id_generator,
        ...
        assert len(result.rows) == 1
        assert result.rows[0].drug_concept_id == 1124957
    ...
    def test_missing_medication_name_skips(
        self,
        static_index,
        structural_index,
        row_id_generator,
    ...
    def test_end_date_falls_back_to_start_date(
        ...
        assert len(result.rows) == 1
        assert result.rows[0].drug_exposure_start_date == dt.date(2023, 1, 10)
        assert result.rows[0].drug_exposure_end_date == dt.date(2023, 1, 10)

    def test_multi_ingredient_produces_multiple_result(
        self,
        static_index,
        structural_index,
        row_id_generator,
        ...
        assert result.rows[0].drug_exposure_id != result.rows[1].drug_exposure_id
        assert all(r.drug_source_value == "Calcigran Forte" for r in result.rows)


class TestIdUniqueness:
    def test_ids_unique_across_sources(
        self,
        static_index,
        structural_index,
        row_id_generator,
        ...
        ids = [r.drug_exposure_id for r in result.rows]
    ...
    def test_ids_deterministic(
        ...
        builder = DrugExposureBuilder(ConceptLookupService(static_index, structural_index), row_id_generator)
        result1 = builder.build(create_build_context(patient, PERSON_ID))
...
class TestTreatmentCyclePublication:
    ...
    def test_publishes_cycle_drug_exposure_row(
        self,
        static_index,
        structural_index,
        row_id_generator,
        ...
        rows = DrugExposureBuilder(ConceptLookupService(static_index, structural_index), row_id_generator).build_and_populate(ctx)

        refs = ctx.resolve_rows(OmopTables.DRUG_EXPOSURE, self._cycle_ref(cycle))
        assert len(refs) == 1
        assert refs[0].row_id == rows[0].drug_exposure_id

    def test_multi_concept_cycle_publishes_all_rows(
        self,
        static_index,
        structural_index,
        row_id_generator,
        ...
        ctx = create_build_context(patient, PERSON_ID)
        rows = DrugExposureBuilder(ConceptLookupService(static_index, structural_index, semantic), row_id_generator).build_and_populate(ctx)
