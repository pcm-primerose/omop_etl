...
class TestPersonBuilder:
    def test_all_fields_populated(
        ...
        result = PersonBuilder(concepts, row_id_generator).build(create_build_context(patient, person_id))
    ...
    def test_female_sex_maps_correctly(
        self,
        static_index,
        structural_index,
        row_id_generator,
        ...
        result = PersonBuilder(concepts, row_id_generator).build(create_build_context(patient))

        assert len(result.rows) == 1
        assert result.rows[0].gender_concept_id == 8532
        assert result.rows[0].gender_source_value == "f"

    def test_missing_dob_returns_empty(
        self,
        static_index,
        structural_index,
        row_id_generator,
    ...
    def test_missing_sex_uses_concept_id_zero(
        self,
        static_index,
        structural_index,
        row_id_generator,
    ...
    def test_row_id_is_deterministic(
        ...
        rows_a = PersonBuilder(concepts, row_id_generator).build(create_build_context(patient, person_id))
        rows_b = PersonBuilder(concepts, row_id_generator).build(create_build_context(patient, person_id))
