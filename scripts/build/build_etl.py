...
Build the ETL container.
    ...
    uv run scripts/build/build_etl.py arm64   native image, for local testing
    uv run scripts/build/build_etl.py amd64   cluster image, plus a docker-archive tar
    uv run scripts/build/build_etl.py sif     apptainer image, from the amd64 tar
    uv run scripts/build/build_etl.py all     all of the above
"""
...
IMAGE = os.environ.get("IMAGE", "omop-etl")
TAG = os.environ.get("TAG", "latest")
DOCKERFILE = ROOT / "docker" / "etl.Dockerfile"
...
def build_sif() -> None:
    if not TAR.exists():
        raise SystemExit(f"missing {TAR}; run 'build_etl.py amd64' first")
